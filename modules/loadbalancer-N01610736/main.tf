...
resource "azurerm_loadbalancer_backend_address_pool" "loadbalancer-backend-address_pool" {
  loadbalancer_id = azurerm_loadbalancer.loadbalancer.id
  name            = "${var.loadbalancer-name}-address_pool"
 
}

resource "azurerm_loadbalancer_probe" "probe" {
...
resource "azurerm_loadbalancer_rule" "loadbalancer-rules" {
  loadbalancer_id                = azurerm_loadbalancer.loadbalancer.id
  name                           = "http-rule"
  protocol                       = "Tcp"
  frontend_port                  = 80
  ...
  backend_address_pool_ids       = [azurerm_loadbalancer_backend_address_pool.loadbalancer-backend-address_pool.id]
  probe_id                       = azurerm_loadbalancer_probe.probe.id
...
resource "azurerm_network_interface_backend_address_pool_association" "loadbalancer-nic-backend_pool_association" {
  ...
  backend_address_pool_id = azurerm_loadbalancer_backend_address_pool.loadbalancer-backend-address_pool.id
